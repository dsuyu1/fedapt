...
Client task data comprises raw telemetry from public, paper-backed log datasets---the AIT Log Data Set~\cite{landauer2022ait} (matched benign/attack enterprise logs), CIC-IDS2017~\cite{sharafaldin2017cicids} and UNSW-NB15~\cite{moustafa2015unsw} (network flows), and Stratus Red Team CloudTrail detonations~\cite{stratus2023}, optionally alongside \texttt{splunk/attack\_data}---each converted to a common record schema and, where applicable, supplying both the attack and benign classes from a single matched environment. \emph{These public datasets are partitioned across simulated organizations to stand in for genuinely private per-organization data.} Telemetry spans three domains: \emph{Endpoint} (Sysmon, WinEvent~4688, PowerShell), \emph{Network} (Zeek, Suricata, firewall), and \emph{Cloud} (AWS CloudTrail, Azure, Okta). To simulate enterprise heterogeneity, data is partitioned non-IID across $N{=}6$ clients using a Dirichlet distribution $\boldsymbol{p}\sim\mathrm{Dir}(\alpha)$ over domains ($\alpha{=}0.5$); small $\alpha$ concentrates domains within individual clients. Each source is converted once, offline, into a common record schema $\langle\text{log},\text{domain},\text{technique},\text{is\_malicious}\rangle$ and merged through a single pluggable interface, so the pipeline consumes any combination of datasets (and optionally \texttt{attack\_data}) without source-specific code.

\paragraph{Label granularity (disclosure).} Sources differ in label resolution: AIT labels individual log lines, whereas \texttt{attack\_data} labels whole captures. We normalize both to a common \emph{record} level by windowing each log into fixed-length segments and labeling a segment \texttt{attack} iff it contains at least one attack-labeled line (for \texttt{attack\_data}, the entire record inherits its capture label). We therefore frame the verdict task at the record level---\emph{``does this activity contain evidence of attack activity?''} (labels \texttt{attack}/\texttt{benign})---rather than a per-line malicious/benign judgment, which the coarser sources do not support and which would inject label noise.
...
\paragraph{Implementation and reproducibility.} The framework is a tested Python package driven by a single \texttt{Config}. Data preparation, federated Stage~1, Stage~2 task tuning, evaluation, and analysis run as independent, \emph{resumable} stages---completed adapters and result files are skipped on re-run---via command-line scripts or thin notebooks (Colab GPU). Public datasets enter through one-time converters into the common record schema; teacher and judge models are pluggable and may be API-hosted or served locally, enabling fully offline, no-cost runs.

\begin{table}[t]
\centering
\caption{Realized Stage-1 prose corpus (public sources).}
\label{tab:data}
\begin{tabular}{lr}
\toprule
Source & Documents \\
...
The Stage-2 log-grounded tasks (\texttt{explain\_log}, \texttt{verdict}) and the two prose tasks (\texttt{explain\_example}, \texttt{general\_qa}) are constructed from the matched both-class datasets described above through the ingestion layer; per-dataset record counts and the teacher-synthesized targets are finalized at build time and reported with results.

\begin{figure}[t]
\centering
% produced by scripts/analyze.py (data_report/data_overview.png)
\includegraphics[width=0.95\linewidth]{data_overview.png}
\caption{Data distribution: prose corpus by source, non-IID prose shards per client, log records per client, and log records by security domain. The heavy per-client skew is the intended non-IID setting. The log panels illustrate the partitioning mechanism and are regenerated for each dataset build.}
\label{fig:data}
\end{figure}
